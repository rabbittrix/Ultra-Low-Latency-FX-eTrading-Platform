...
export default function ExecutionsPage() {
  ...
  return (
    <div className="p-6">
      ...
      {error && (
        <div className="mb-4 space-y-2 rounded border border-red-700 bg-red-900/50 p-4 text-red-200">
          <p className="font-medium">Could not load trades</p>
          <p className="break-all font-mono text-sm opacity-90">{error}</p>
          ...
          {gatewayHealth?.reachable && /\b404\b/.test(error) && (
            <p className="text-xs text-amber-200/95">
              ...
              <code className="text-amber-100">/matching/trades</code> returned 404. Rebuild and
              restart <code className="text-amber-100">gateway-service</code> (proxy uses{' '}
              <code className="text-amber-100">OriginalUri</code>
              ), and run <code className="text-amber-100">matching-engine-service</code> on{' '}
              <code className="text-amber-100">8083</code> (
              <code className="text-amber-100">npm run dev:stack</code>).
            </p>
          )}
          ...
          <p className="text-xs text-red-300/80">
            From <code className="text-red-200">frontend/tauri-trading-ui</code>, run{' '}
            <code className="text-red-200">npm run dev:stack</code> (UI + gateway + matching), or{' '}
            <code className="text-red-200">docker compose up -d</code> in{' '}
            <code className="text-red-200">deploy/</code>.
          </p>
        </div>
      )}

      {!error && trades.length === 0 && (
        <div className="mb-4 rounded border border-gray-800 bg-gray-900 p-4 text-sm text-gray-400">
              ...
              trades.map((trade) => (
                <tr key={trade.trade_id} className="border-t border-gray-800 hover:bg-gray-800/50">
                  ...
                  <td className="px-4 py-3 text-sm text-green-400">
                    {trade.buy_order_id.slice(0, 8)}...
                  </td>
                  <td className="px-4 py-3 text-sm text-red-400">
                    {trade.sell_order_id.slice(0, 8)}...
                  </td>
