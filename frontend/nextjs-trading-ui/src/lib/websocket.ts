        ...
        this.ws.onerror = () => {
          ...
          if (!this.openedSuccessfully) {
            warnGatewayUnreachableThrottled(
              `[WebSocket] no gateway at ${failUrl} — run \`npm run dev:stack\` from frontend/nextjs-trading-ui, or \`cargo run --bin gateway-service\` (8080), or set NEXT_PUBLIC_API_URL.`,
            );
          }
          this.isConnecting = false;
